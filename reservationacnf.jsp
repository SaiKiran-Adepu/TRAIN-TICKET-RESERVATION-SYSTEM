...
<%!
    ...
    public void jspInit()
    {
        ...
        { 
            ...
            ps1 = con.prepareStatement("insert into train_reservation(Train_Number,Train_Name,Frm,To1,Classc,Ticket_Type,Passenger_Names,Passengers_age,Adult,Child,Couch,Berth,Base_Fare,Class_Fare,Quota_Fare,Fare,Km,Booked_on,Booked_For,Booked_By,tad,Gender,Pay_mode)values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
            ...
            ps2 = con.prepareStatement("select * from train_reservation where Booked_On=? AND tad=? ORDER BY Booked_On");
              ps3 = con.prepareStatement("INSERT INTO train_sitting(name, age, gender, berth,tusr,tadm,tbd) VALUES (?, ?, ?, ?, ?, ?, ?)");
            ps4 = con.prepareStatement("select * from train_sitting where tbd=? AND tadm=? ORDER BY tbd DESC");
            String query = "SELECT CONCAT( TIME_FORMAT(STR_TO_DATE(Booked_On, '%H:%i:%s %Y-%m-%d'), '%h:%i:%s %p'), ' ', DATE_FORMAT(STR_TO_DATE(Booked_On, '%H:%i:%s %Y-%m-%d'), '%d-%b-%Y') ) AS Book, DATE_FORMAT(STR_TO_DATE(Booked_For, '%Y-%m-%d'), '%d-%b-%Y') AS journey FROM train_reservation WHERE Booked_On=? AND tad=?";
...
<% 
       ...
       String pge1 = request.getParameter("page1");
        ...
        int km=Integer.parseInt(request.getParameter("km"));
         ...
         String usr = request.getParameter("usr");
           String adm = request.getParameter("adm1");
    ...
    String[] pageValues = request.getParameterValues("page2[]");
    String[] pgenValues = request.getParameterValues("pgen2[]");
    String[] pbthValues = request.getParameterValues("pbth2[]");
    ...
    out.println("</table></center>");
           ...
           ps2.setString(2,adm1);

           ps4.setString(1,dob1);
           ps4.setString(2,adm1);
...
while (rs2.next()) {
  out.println("<tr>");
       ...
       String dobt = request.getParameter("dob");
            ...
            String admt = request.getParameter("adm1");
           ps5.setString(1,dobt);
           ps5.setString(2,admt);
...
out.println("</tr>");
    ...
    while (rs3.next()) 
    {
        ...
        out.println("<td>&nbsp&nbspCNF/"+ rs2.getString(12) +"/"+rs3.getInt(5)+"/"+rs3.getString(6)+"&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp CNF/"+ rs2.getString(12) +"/"+rs3.getInt(5)+"/"+rs3.getString(6)+ "</td>");
        out.println("</tr>");
       ...
       out.println("</tr>");
        ...
        out.println("<td><b>Class Charges:&nbsp</b>"+acc+"</td>");
         out.println("<td><b>Quota Charges:&nbsp</b>"+tc+"</td>");
          out.println("<td><b>Total RS:&nbsp</b>"+fr+"&nbsp<b>Payment Mode:&nbsp</b>"+pm+"</td>");
          out.println("</tr>");
